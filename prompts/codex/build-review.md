...
- Only when the human asks for a Codex review, run `codex review "<prompt>"` once with a focused prompt identifying the agreed review target and specific concern. Finish any intended changes, validation, or commits before running it. Share the full results with the human and stop — do not act on the feedback or rerun review autonomously. If review fails, report the failure and let the human decide how to proceed.
