...
Establish the target repository and both branch inputs before analysis. Ask for missing branch names. The factory merge launcher checks for a clean working tree, fetches origin, and resolves the two branches, including creating local branches for remote-only inputs. When this prompt is used directly, do not assume those checks ran: establish the repository state, fetch origin, and resolve both branches before analysis. If the working tree is dirty or a prerequisite fails, report it and let the human resolve it; do not stash, discard changes, or analyze against assumed branch state. Record the resolved SHAs for the plan. These checks do not authorize a merge or changes to either branch's content.
...
5. **For changes without plans**, read commit messages and code. Context hierarchy: plans > commit messages > code.
...
- During planning, write only the uncommitted merge plan; do not modify source code or commit on either source branch. After human approval, carry the plan onto the new merge branch and commit it there.
